        ...
        <div className="max-w-md mx-auto space-y-6 sm:space-y-8">
          ...
          <div className="pt-4 border-t border-linen">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              <button onClick={() => setEmotions(DEFAULT_EMOTIONS.map(e => ({ ...e, value: 30 })))} className="px-3 py-1.5 text-xs bg-sage/20 text-text-secondary rounded-full hover:bg-sage/30 transition-colors">Feeling Good</button>
              <button onClick={() => setEmotions(DEFAULT_EMOTIONS.map(e => ({ ...e, value: 70 })))} className="px-3 py-1.5 text-xs bg-terracotta/20 text-text-secondary rounded-full hover:bg-terracotta/30 transition-colors">Stressed</button>
              <button onClick={() => setEmotions(DEFAULT_EMOTIONS.map(e => ({ ...e, value: 50 })))} className="px-3 py-1.5 text-xs bg-linen text-text-secondary rounded-full hover:bg-cream transition-colors">Neutral</button>
            </div>
          </div>
        </div>
      </div>
...
export function EmotionScaleSliders({ onClose, onSave, initialEmotions, title = "How are you feeling?" }: EmotionScaleSlidersProps) {
  const [emotions, setEmotions] = useState<EmotionScale[]>(() => DEFAULT_EMOTIONS.map(e => ({ ...e, value: 5 })))
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-white z-10 flex flex-col">
      ...
      <div className="px-4 sm:px-6 py-4 border-t border-linen bg-cream/50">
        <Button onClick={handleSave} className="w-full" size="lg">Save Check-in</Button>
        <p className="text-xs text-text-muted text-center mt-2">Tracking your emotions helps {emotions[0]?.label ? "understand your patterns" : "you"} over time</p>
      </div>
    </motion.div>
  )
}
