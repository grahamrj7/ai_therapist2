        ...
        <div className="max-w-lg mx-auto space-y-6">
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard
              ...
              sublabel={goals.length > 0 ? `${completionRate}% complete` : "No goals yet"}
            />
            <StatCard
              icon={<BookOpen className="h-4 w-4" />}
              label="Journal Entries"
...
export function ProgressOverview({ userId, onClose }: ProgressOverviewProps) {
  ...
  const journalEntriesLast30Days = journalEntries.filter((e) => e.created_at >= thirtyDaysAgo).length
        ...
        <div className="max-w-lg mx-auto space-y-6">
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard
              icon={<BookOpen className="h-4 w-4" />}
              label="Journal Entries"
              value={journalEntriesLast30Days}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {activeGoals.slice(0, 3).map((goal) => (
                  <div key={goal.id} className="rounded-xl border border-linen p-3 bg-white">
...
import { useState, useEffect } from "react"
import { motion } from "framer-motion"
import { TrendingUp, X, Flame, Target, BookOpen, MessageCircle, Loader2, CheckCircle2 } from "lucide-react"
import { loadGoals, loadJournalEntries, loadSessions, type Goal, type JournalEntry } from "@/lib/db"
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {recentlyCompleted.map((goal) => (
                  <div
                    key={goal.id}
                    className="flex items-center gap-3 rounded-xl border border-linen p-3 bg-cream/30"
                  >
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        <div className="max-w-lg mx-auto space-y-6">
          ...
          {goals.length === 0 && journalEntries.length === 0 && totalSessions === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="font-medium text-text-primary mb-2">No activity yet</h3>
              <p className="text-sm text-text-muted">
                Your progress will show up here automatically as you chat, journal, and set goals.
              </p>
            </div>
          )}
        </div>
